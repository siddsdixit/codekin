 ...
 */
async function extractTextFromFile(filePath, maxReadFileLine) {
    ...
    if (!isBinary) {
        ...
        if (maxReadFileLine !== undefined && maxReadFileLine !== -1) {
            const totalLines = await (0, line_counter_1.countFileLines)(filePath);
            if (totalLines > maxReadFileLine) {
                // Read only up to maxReadFileLine (endLine is 0-based and inclusive)
                const content = await (0, read_lines_1.readLines)(filePath, maxReadFileLine - 1, 0);
