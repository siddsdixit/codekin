...
class UseMcpToolTool extends BaseTool_1.BaseTool {
    ...
    parseLegacy(params) {
        ...
        return {
            ...
            arguments: params.arguments, // Keep as string for validation to handle
        };
    }
        ...
        if (params.arguments) {
            ...
            if (typeof params.arguments === "object") {
                ...
                try {
                    ...
                    pushToolResult(responses_1.formatResponse.toolError(responses_1.formatResponse.invalidMcpToolArgumentError(params.server_name, params.tool_name)));
                    return { isValid: false };
                }
            }
        }
    ...
    async validateToolExists(task, serverName, toolName, pushToolResult) {
        try {
            ...
            if (!server.tools || server.tools.length === 0) {
                ...
                task.consecutiveMistakeCount++;
                task.recordToolError("use_mcp_tool");
                await task.say("error", (0, i18n_1.t)("mcp:errors.toolNotFound", {
                    toolName,
                    serverName,
            ...
            if (tool.enabledForPrompt === false) {
                ...
                task.consecutiveMistakeCount++;
                task.recordToolError("use_mcp_tool");
                await task.say("error", (0, i18n_1.t)("mcp:errors.toolDisabled", {
                    toolName,
                    serverName,
