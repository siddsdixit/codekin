...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		case "webviewDidLaunch":
			try {
				provider.log("[webviewMessageHandler] webviewDidLaunch received")
				// Load custom modes first
				const customModes = await provider.customModesManager.getCustomModes()
				await updateGlobalState("customModes", customModes)

				provider.log("[webviewMessageHandler] Posting state to webview...")
				await provider.postStateToWebview()
				provider.log("[webviewMessageHandler] State posted successfully")
			...
			provider.log("[webviewMessageHandler] webviewDidLaunch completed successfully")
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : String(error)
				const errorStack = error instanceof Error ? error.stack : undefined
				provider.log(`[webviewMessageHandler] ❌ ERROR in webviewDidLaunch: ${errorMessage}`)
				if (errorStack) {
					provider.log(`[webviewMessageHandler] Stack trace: ${errorStack}`)
				}
				// Try to show error to user
				vscode.window.showErrorMessage(`Failed to initialize Codekin webview: ${errorMessage}`)
				// Still set the flag so we don't get stuck
				provider.isViewLaunched = true
			}
			break
		case "newTask":
