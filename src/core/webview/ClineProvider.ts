	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	*/
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		try {
			this.outputChannel.appendLine("[Codekin] resolveWebviewView called - webview is being resolved!")
			...
			this.getState()
				...
				.catch((error) => {
					this.outputChannel.appendLine(
						`[Codekin] Error loading terminal state: ${error instanceof Error ? error.message : String(error)}`,
					)
				})
			...
			this.getState()
				...
				.catch((error) => {
					this.outputChannel.appendLine(
						`[Codekin] Error loading TTS enabled state: ${error instanceof Error ? error.message : String(error)}`,
					)
				})
			...
			this.getState()
				...
				.catch((error) => {
					this.outputChannel.appendLine(
						`[Codekin] Error loading TTS speed state: ${error instanceof Error ? error.message : String(error)}`,
					)
				})
			...
			const isDevelopment = this.contextProxy.extensionMode === vscode.ExtensionMode.Development
			this.outputChannel.appendLine(
				`[Codekin] Setting webview HTML (mode: ${isDevelopment ? "Development" : "Production"})`,
			)
			...
			try {
				...
				if (!isDevelopment) {
					const scriptMatch = htmlContent.match(/src="([^"]+index\.js[^"]*)"/)
					if (scriptMatch) {
						this.outputChannel.appendLine(`[Codekin] Production script URI: ${scriptMatch[1]}`)
					}
				}
				
				webviewView.webview.html = htmlContent
				this.outputChannel.appendLine("[Codekin] Webview HTML set successfully")
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : String(error)
				const errorStack = error instanceof Error ? error.stack : undefined
				this.outputChannel.appendLine(`[Codekin] ❌ ERROR: Failed to generate webview HTML: ${errorMessage}`)
				if (errorStack) {
					this.outputChannel.appendLine(`[Codekin] Stack trace: ${errorStack}`)
				}
				// Set a basic error HTML so the user sees something
				webviewView.webview.html = `<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<title>Codekin - Error</title>
	<style>
...
<body>
	<h1>⚠️ Webview Loading Error</h1>
	<p>Failed to load the Codekin webview. Please check the Output panel for details.</p>
	<pre>${errorMessage}</pre>
	<p>Try reloading the window (Cmd+Shift+P → "Developer: Reload Window")</p>
</body>
</html>`
			...
			try {
				...
				this.outputChannel.appendLine("[Codekin] Webview message listener set successfully")
			} catch (error) {
				this.outputChannel.appendLine(
					`[Codekin] ❌ ERROR: Failed to set webview message listener: ${error instanceof Error ? error.message : String(error)}`,
				)
				throw error
			}
			...
			try {
				...
				this.outputChannel.appendLine(
					`[Codekin] Warning: Failed to update code index status subscription: ${error instanceof Error ? error.message : String(error)}`,
				)
			}
			...
			const activeEditorSubscription = vscode.window.onDidChangeActiveTextEditor(() => {
				...
				try {
					...
					this.outputChannel.appendLine(
						`[Codekin] Warning: Failed to update code index status on editor change: ${error instanceof Error ? error.message : String(error)}`,
					)
				}
			})
			...
			const configDisposable = vscode.workspace.onDidChangeConfiguration(async (e) => {
				if (e && e.affectsConfiguration("workbench.colorTheme")) {
					...
					try {
						...
						this.outputChannel.appendLine(
							`[Codekin] Warning: Failed to send theme update: ${error instanceof Error ? error.message : String(error)}`,
						)
					}
				}
			})
			...
			try {
				...
				this.outputChannel.appendLine(
					`[Codekin] Warning: Failed to remove Cline from stack: ${error instanceof Error ? error.message : String(error)}`,
				)
			}
			...
			const errorMessage = error instanceof Error ? error.message : String(error)
			const errorStack = error instanceof Error ? error.stack : undefined
			this.outputChannel.appendLine(`[Codekin] ❌ CRITICAL ERROR in resolveWebviewView: ${errorMessage}`)
			if (errorStack) {
				this.outputChannel.appendLine(`[Codekin] Stack trace: ${errorStack}`)
			}
			// Show error to user
			vscode.window.showErrorMessage(`Failed to initialize Codekin webview: ${errorMessage}`)
			throw error
		}
	}
	...
	public async postMessageToWebview(message: ExtensionMessage) {
		try {
			if (!this.view) {
				this.outputChannel.appendLine(
					`[Codekin] Warning: Cannot post message to webview - view is not available. Message type: ${message.type}`,
				)
				return
			}
			if (!this.view.webview) {
				this.outputChannel.appendLine(
					`[Codekin] Warning: Cannot post message to webview - webview is not available. Message type: ${message.type}`,
				)
				return
			}
			...
			const errorMessage = error instanceof Error ? error.message : String(error)
			this.outputChannel.appendLine(
				`[Codekin] ❌ ERROR: Failed to post message to webview (type: ${message.type}): ${errorMessage}`,
			)
	...
	private async getHtmlContent(webview: vscode.Webview): Promise<string> {
		try {
			...
			const imagesUri = getUri(webview, this.contextProxy.extensionUri, ["assets", "images"])
			const audioUri = getUri(webview, this.contextProxy.extensionUri, ["dist", "webview", "audio"])
		...
		*/
		...
		// Get the OpenRouter base URL from configuration
		let openRouterBaseUrl = "https://openrouter.ai"
		let openRouterDomain = "https://openrouter.ai"
		try {
			const { apiConfiguration } = await this.getState()
			openRouterBaseUrl = apiConfiguration.openRouterBaseUrl || "https://openrouter.ai"
			// Extract the domain for CSP
			openRouterDomain = openRouterBaseUrl.match(/^(https?:\/\/[^\/]+)/)?.[1] || "https://openrouter.ai"
		} catch (error) {
			this.outputChannel.appendLine(
				`[Codekin] Warning: Failed to get state for OpenRouter URL, using default: ${error instanceof Error ? error.message : String(error)}`,
			)
		}
          ...
          <body>
            ...
            <script nonce="${nonce}">
              ...
              window.addEventListener('error', (event) => {
                console.error('[Codekin] Global error:', event.error);
                console.error('[Codekin] Error message:', event.message);
                console.error('[Codekin] Error source:', event.filename, ':', event.lineno);
              ...
              window.addEventListener('unhandledrejection', (event) => {
                console.error('[Codekin] Unhandled promise rejection:', event.reason);
                if (window.vscode && window.vscode.postMessage) {
                  window.vscode.postMessage({
                    type: 'error',
              ...
              console.log('[Codekin] Webview HTML loaded');
              console.log('[Codekin] Script URI:', '${scriptUri}');
              console.log('[Codekin] Styles URI:', '${stylesUri}');
              console.log('[Codekin] Root element:', document.getElementById('root'));
              console.log('[Codekin] VSCode API available:', typeof window.vscode !== 'undefined');
              
              // Check if script loads
              const script = document.querySelector('script[type="module"]');
              if (script) {
                script.addEventListener('load', () => {
                  console.log('[Codekin] ✅ Script loaded successfully');
                });
                script.addEventListener('error', (e) => {
                  console.error('[Codekin] ❌ Script failed to load:', e);
                });
              }
            </script>
      ...
      `
			...
			const errorMessage = error instanceof Error ? error.message : String(error)
			this.outputChannel.appendLine(`[Codekin] ❌ ERROR in getHtmlContent: ${errorMessage}`)
			throw new Error(`Failed to generate webview HTML: ${errorMessage}`)
		}
	}
	...
	async postStateToWebview() {
		try {
			if (!this.view) {
				this.outputChannel.appendLine("[Codekin] Warning: Cannot post state - webview view is not available")
				return
			}

			this.outputChannel.appendLine("[Codekin] Getting state to post to webview...")
			const state = await this.getStateToPostToWebview()
			this.outputChannel.appendLine("[Codekin] State retrieved, posting to webview...")
			
			this.postMessageToWebview({ type: "state", state })
			this.outputChannel.appendLine("[Codekin] ✓ State posted to webview successfully")
			...
			const errorMessage = error instanceof Error ? error.message : String(error)
			const errorStack = error instanceof Error ? error.stack : undefined
			this.outputChannel.appendLine(`[Codekin] ❌ ERROR: Failed to post state to webview: ${errorMessage}`)
			if (errorStack) {
				this.outputChannel.appendLine(`[Codekin] Stack trace: ${errorStack}`)
			}
			// Try to show error to user
			vscode.window.showErrorMessage(`Failed to initialize Codekin: ${errorMessage}`)
			throw error
		}
	}
