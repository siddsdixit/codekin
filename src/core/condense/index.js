...
exports.MAX_CONDENSE_THRESHOLD = exports.MIN_CONDENSE_THRESHOLD = exports.N_MESSAGES_TO_KEEP = void 0;
 ...
 * @returns {SummarizeResponse} - The result of the summarization operation (see above)
 */
async function summarizeConversation(messages, apiHandler, systemPrompt, taskId, prevContextTokens, isAutomaticTrigger, customCondensingPrompt, condensingApiHandler) {
    telemetry_1.TelemetryService.instance.captureContextCondensed(taskId, isAutomaticTrigger ?? false, !!customCondensingPrompt?.trim(), !!condensingApiHandler);
    ...
    const messagesToSummarize = getMessagesSinceLastSummary(messages.slice(0, -exports.N_MESSAGES_TO_KEEP));
    if (messagesToSummarize.length <= 1) {
        const error = messages.length <= exports.N_MESSAGES_TO_KEEP + 1
            ? (0, i18n_1.t)("common:errors.condense_not_enough_messages")
            : (0, i18n_1.t)("common:errors.condensed_recently");
        return { ...response, error };
    }
    const keepMessages = messages.slice(-exports.N_MESSAGES_TO_KEEP);
    ...
    const requestMessages = (0, image_cleaning_1.maybeRemoveImageBlocks)([...messagesToSummarize, finalRequestMessage], apiHandler).map(({ role, content }) => ({ role, content }));
    ...
    if (newContextTokens >= prevContextTokens) {
        const error = (0, i18n_1.t)("common:errors.condense_context_grew");
        return { ...response, cost, error };
    }
