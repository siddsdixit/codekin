...
// Plugin to handle CommonJS modules from shared directory
const sharedCommonJsPlugin = (): Plugin => ({
	name: "shared-commonjs",
	async resolveId(id, importer) {
		// If importing from @roo alias and it resolves to a .js file, prefer .ts instead
		if (id.startsWith("@roo/") && importer) {
			const sharedPath = resolve(__dirname, "../src/shared")
			const moduleName = id.replace("@roo/", "")
			const tsPath = resolve(sharedPath, `${moduleName}.ts`)
			const jsPath = resolve(sharedPath, `${moduleName}.js`)
			
			// Prefer .ts over .js in development
...
export default defineConfig(({ mode }) => {
	...
	const define: Record<string, any> = {
		...
		"process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("Codekin"),
	...
	return {
		...
		resolve: {
			...
			// Prefer TypeScript sources over compiled JS in development
			extensions: mode === "development" 
				? [".ts", ".tsx", ".mts", ".mjs", ".js", ".jsx", ".json"]
		...
		build: {
			...
			commonjsOptions: {
				...
				// Ensure CommonJS modules in shared directory are properly transformed
