...
function truncateConversation(messages, fracToRemove, taskId) {
    telemetry_1.TelemetryService.instance.captureSlidingWindowTruncation(taskId);
 ...
 */
async function manageContext({ messages, totalTokens, contextWindow, maxTokens, apiHandler, autoCondenseContext, autoCondenseContextPercent, systemPrompt, taskId, customCondensingPrompt, condensingApiHandler, profileThresholds, currentProfileId, }) {
    let error;
    let cost = 0;
    ...
    if (profileThreshold !== undefined) {
        ...
        else if (profileThreshold >= condense_1.MIN_CONDENSE_THRESHOLD && profileThreshold <= condense_1.MAX_CONDENSE_THRESHOLD) {
    ...
    if (autoCondenseContext) {
        ...
        if (contextPercent >= effectiveThreshold || prevContextTokens > allowedTokens) {
            ...
            const result = await (0, condense_1.summarizeConversation)(messages, apiHandler, systemPrompt, taskId, prevContextTokens, true, // automatic trigger
