...
function getEditingInstructions(mode, customModes, experiments, codeIndexManager, settings, diffStrategy) {
    ...
    const hasApplyDiff = diffStrategy && availableEditTools.includes("apply_diff");
    const hasWriteToFile = availableEditTools.includes("write_to_file");
    const hasInsertContent = availableEditTools.includes("insert_content");
...
function getRulesSection(cwd, supportsComputerUse, mode, customModes, experiments, diffStrategy, codeIndexManager, settings) {
...
- All file paths must be relative to this directory. However, commands may change directories in terminals, so respect working directory specified by the response to ${(0, types_1.isNativeProtocol)(effectiveProtocol) ? "execute_command" : "<execute_command>"}.
...
${codebaseSearchRule}${editingToolsRef
...
- Your goal is to try to accomplish the user's task, NOT engage in a back and forth conversation.${hasBrowserAction
...
- It is critical you wait for the user's response after each tool use, in order to confirm the success of the tool use. For example, if asked to make a todo app, you would create a file, wait for the user's response it was created successfully, then create another file if needed, wait for the user's response it was created successfully, etc.${hasBrowserAction
