...
const browserDiscovery_1 = require("./browserDiscovery");
// Timeout constants
...
class BrowserSession {
    ...
    async connectWithChromeHostUrl(chromeHostUrl) {
        try {
            this.browser = await (0, puppeteer_core_1.connect)({
    ...
    async connectToRemoteBrowser() {
        ...
        // If user provided a remote browser host, try to connect to it
            ...
            try {
                const hostIsValid = await (0, browserDiscovery_1.tryChromeHostUrl)(remoteBrowserHost);
                if (!hostIsValid) {
