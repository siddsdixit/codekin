...
async function applyDiffTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    if ((0, types_2.isNativeProtocol)(toolProtocol)) {
        return ApplyDiffTool_1.applyDiffTool.handle(cline, block, {
            askApproval,
            handleError,
            pushToolResult,
            removeClosingTag,
    ...
    if (argsXmlTag) {
        ...
        try {
            ...
            const parsed = (0, xml_1.parseXmlForDiff)(argsXmlTag, ["file.diff.content"]);
...
Original error: ${errorMessage}`;
            ...
            telemetry_1.TelemetryService.instance.captureDiffApplicationError(cline.taskId, cline.consecutiveMistakeCount);
    ...
    try {
        ...
        for (const operation of operations) {
            ...
            if (!accessAllowed) {
                ...
                updateOperationResult(relPath, {
                    status: "blocked",
                    error: responses_1.formatResponse.rooIgnoreError(relPath),
                });
                continue;
            }
        ...
        if (operationsToApprove.length > 1) {
            ...
            for (const opResult of operationsToApprove) {
                const readablePath = (0, path_2.getReadablePath)(cline.cwd, opResult.path);
                const changeCount = opResult.diffItems?.length || 0;
                const changeText = changeCount === 1 ? "1 change" : `${changeCount} changes`;
                let unified = "";
                try {
                    const original = await promises_1.default.readFile(opResult.absolutePath, "utf-8");
                    const processed = !cline.api.getModel().id.includes("claude")
                        ? (opResult.diffItems || []).map((item) => ({
                            ...item,
                            content: item.content ? (0, text_normalization_1.unescapeHtmlEntities)(item.content) : item.content,
                        }))
                        : opResult.diffItems || [];
                    const applyRes = (await cline.diffStrategy?.applyDiff(original, processed)) ?? { success: false };
                    const newContent = applyRes.success && applyRes.content ? applyRes.content : original;
                    unified = responses_1.formatResponse.createPrettyPatch(opResult.path, original, newContent);
                }
                catch {
                    unified = "";
                }
                const unifiedSanitized = (0, stats_1.sanitizeUnifiedDiff)(unified);
                const stats = (0, stats_1.computeDiffStats)(unifiedSanitized) || undefined;
                batchDiffs.push({
                    path: readablePath,
                    changeCount,
        ...
        for (const opResult of operationResults) {
            ...
            try {
                let originalContent = await promises_1.default.readFile(absolutePath, "utf-8");
                let beforeContent = originalContent;
                let successCount = 0;
                let formattedError = "";
                ...
                if (!diffResult.success) {
                    ...
                    telemetry_1.TelemetryService.instance.captureDiffApplicationError(cline.taskId, currentCount);
...
${errorDetails ? `\nTechnical details:\n${errorDetails}\n` : ""}
                ...
                if (operationsToApprove.length === 1) {
                    ...
                    const unifiedPatchRaw = responses_1.formatResponse.createPrettyPatch(relPath, beforeContent, originalContent);
                    ...
                    const operationMessage = JSON.stringify({
                        ...
                        diffStats: (0, stats_1.computeDiffStats)(unifiedPatch) || undefined,
                    });
