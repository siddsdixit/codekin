...
class TerminalProcess extends BaseTerminalProcess_1.BaseTerminalProcess {
    ...
    async run(command) {
        ...
        const streamAvailable = new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                ...
                this.emit("no_shell_integration", `VSCE shell integration stream did not start within ${Terminal_1.Terminal.getShellIntegrationTimeout() / 1000} seconds. Terminal problem?`);
                // Reject with descriptive error
                reject(new Error(`VSCE shell integration stream did not start within ${Terminal_1.Terminal.getShellIntegrationTimeout() / 1000} seconds.`));
    ...
    removeEscapeSequences(str) {
        ...
        return (0, strip_ansi_1.default)(str.replace(/\x1b\]633;[^\x07]+\x07/gs, "").replace(/\x1b\]133;[^\x07]+\x07/gs, ""));
    }
    /**
