...
export class CodeIndexManager {
	...
	private async _recreateServices(): Promise<void> {
		// Defensive check: Ensure feature is configured before attempting to create services
		if (!this.isFeatureConfigured) {
			this._stateManager.setSystemState("Standby", "Missing configuration. Save your settings to start indexing.")
			// Return early instead of throwing - this prevents the error from being logged
			return
		}
	...
	public async handleSettingsChange(): Promise<void> {
		if (this._configManager) {
			...
			// If feature is not configured, stop the service
			if (!isFeatureConfigured) {
				// Stop the orchestrator if it exists
				if (this._orchestrator) {
					this._orchestrator.stopWatcher()
				}
				// Set state to indicate service needs configuration
				this._stateManager.setSystemState("Standby", "Missing configuration. Save your settings to start indexing.")
				return
			}
