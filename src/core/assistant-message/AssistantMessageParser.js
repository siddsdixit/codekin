...
exports.AssistantMessageParser = void 0;
...
class AssistantMessageParser {
    ...
    MAX_ACCUMULATOR_SIZE = 1024 * 1024; // 1MB limit
    MAX_PARAM_LENGTH = 1024 * 100; // 100KB per parameter limit
    accumulator = "";
    /**
        ...
        for (let i = 0; i < chunk.length; i++) {
            ...
            if (this.currentToolUse) {
                ...
                if (currentToolValue.endsWith(toolUseClosingTag)) {
                    ...
                    const possibleParamOpeningTags = tools_1.toolParamNames.map((name) => `<${name}>`);
                    for (const paramOpeningTag of possibleParamOpeningTags) {
                        if (this.accumulator.endsWith(paramOpeningTag)) {
                            // Start of a new parameter.
                            const paramName = paramOpeningTag.slice(1, -1);
                            if (!tools_1.toolParamNames.includes(paramName)) {
            ...
            const possibleToolUseOpeningTags = types_1.toolNames.map((name) => `<${name}>`);
            for (const toolUseOpeningTag of possibleToolUseOpeningTags) {
                if (this.accumulator.endsWith(toolUseOpeningTag)) {
                    ...
                    if (!types_1.toolNames.includes(extractedToolName)) {
...
exports.AssistantMessageParser = AssistantMessageParser;
//# sourceMappingURL=AssistantMessageParser.js.map